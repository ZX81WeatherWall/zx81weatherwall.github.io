...
(function (g) {
  ...
  function aggregate2(tiles, opts) {
    ...
    // instruction/headline) signal, matched to a monitored tile by src/wmo-cap.js evacToTiles.
    // opts.evac is that matcher output: [{tile, area}] already deduped + tile-ascending. Keep one
    // per tile, tile-ascending, capped. The `area` (CAP areaDesc readable label) is carried for
    // the ALT prose + tracer; the byte-exact bulletin renders the gazetteer settlement at the tile
    // (precipName, byte-parity with the Z80 reporter — same as tornado). Absent/malformed => empty
    // (an absent feed never fabricates an evacuation; honest — only real CAP evac alerts).
    if (Array.isArray(opts.evac)) {
      const seenEv = new Set();
      for (const m of opts.evac) {
        if (!m) continue;
        const tile = m.tile | 0;
        if (tile < 0 || tile > 99 || seenEv.has(tile)) continue;
        seenEv.add(tile);
        if (s.evac.length < MAX_EVAC_STORE) s.evac.push({ tile, area: (typeof m.area === 'string') ? m.area : '' });
      }
      s.evac.sort((a, b) => a.tile - b.tile);
    }
    // AUTHORITY tornado warnings — NOT a per-tile measured signal (we have no tornado
    // detector). opts.tornadoes is the WMO-CAP matcher output (matchToTiles: already
    // Extreme/Severe + on a monitored tile). Keep ONLY eventCode 9 (TORNADO), one per
    // tile, tile-ascending, capped. Same opts-threading pattern as opts.kp (aurora); the
    // gateway CAP-register -> matcher wire is Phase 2. Absent/malformed => empty (honest:
    // an absent feed never fabricates a tornado).
    if (Array.isArray(opts.tornadoes)) {
      const seenTor = new Set();
      for (const m of opts.tornadoes) {
        if (!m || (m.eventCode | 0) !== TORNADO_EVENT_CODE) continue;
        const tile = m.tile | 0;
        if (seenTor.has(tile)) continue;
        seenTor.add(tile);
        if (s.tornadoes.length < MAX_TORNADO_STORE) s.tornadoes.push({ tile });
      }
      s.tornadoes.sort((a, b) => a.tile - b.tile);
    }
    // AUTHORITY flash-flood warnings — same opts-threading pattern as opts.tornadoes. opts.flashfloods
    // is the merged (WMO+NWS) matchToTiles output (already Extreme/Severe + on a monitored tile). Keep
    // ONLY eventCode 3 (FLASH FLOOD), one per tile, tile-ascending, capped. Absent/malformed => empty
    // (an absent feed never fabricates a warning).
    if (Array.isArray(opts.flashfloods)) {
      const seenFf = new Set();
      for (const m of opts.flashfloods) {
        if (!m || (m.eventCode | 0) !== FLASHFLOOD_EVENT_CODE) continue;
        const tile = m.tile | 0;
        if (seenFf.has(tile)) continue;
        seenFf.add(tile);
        if (s.flashfloods.length < MAX_FLASHFLOOD_STORE) s.flashfloods.push({ tile });
      }
      s.flashfloods.sort((a, b) => a.tile - b.tile);
    }
  ...
  function compose2(stats, timeZ, opts) {
    ...
    // ONCE + cities listed "TORNADO WARNING IN NEW YORK, ..." (FRAG2.IN), cities by population
    // rank then tile (orderPrecip discipline), greedy "+K MORE" under the 300 budget. Both
    ...
    // <place1>, <place2>" for several. Places are the gazetteer settlement at each matched CAP-area
    // tile (precipName — byte-parity with the Z80 reporter, like tornado), by population rank then
    // tile, greedy "+K MORE" under the 300 budget. Both desks. Honest: only real CAP evac alerts
    ...
    const emitEvac = () => {
      const ev = stats.evac || [];
      if (!ev.length) return;
      const items = ev.slice().sort((a, b) => (precipRank(a.tile) - precipRank(b.tile)) || (a.tile - b.tile));
      const leadW = (alertStarted ? F2('SEMI') : F2('ALERT')).length;
    ...
    const emitTornado = () => {
      ...
      if (!tw.length) return;
      const items = tw.slice().sort((a, b) => (precipRank(a.tile) - precipRank(b.tile)) || (a.tile - b.tile));
      const leadW = (alertStarted ? F2('SEMI') : F2('ALERT')).length;
    ...
    // grammar as tornado: "FLASH FLOOD WARNING IN HOUSTON, ..." cities by population rank then tile,
    // greedy "+K MORE" under budget. Both desks. Authority-sourced, carries NO number.
    const emitFlashFlood = () => {
      const ff = stats.flashfloods || [];
      if (!ff.length) return;
      const items = ff.slice().sort((a, b) => (precipRank(a.tile) - precipRank(b.tile)) || (a.tile - b.tile));
      const leadW = (alertStarted ? F2('SEMI') : F2('ALERT')).length;
  ...
  function composeAlt2(stats) {
    ...
    // prose. Prefer the CAP area's own readable label; fall back to the tile region when absent.
    if (stats.evac && stats.evac.length) {
      const places = stats.evac.slice().sort((a, b) => (precipRank(a.tile) - precipRank(b.tile)) || (a.tile - b.tile))
        .map((h) => (h.area && h.area.length ? h.area.toLowerCase() : rn(h.tile)));
    ...
    if (stats.tornadoes && stats.tornadoes.length) {
      const cities = stats.tornadoes.slice().sort((a, b) => (precipRank(a.tile) - precipRank(b.tile)) || (a.tile - b.tile))
        .map((h) => rn(h.tile));
    ...
    if (stats.flashfloods && stats.flashfloods.length) {
      const cities = stats.flashfloods.slice().sort((a, b) => (precipRank(a.tile) - precipRank(b.tile)) || (a.tile - b.tile))
        .map((h) => rn(h.tile));
