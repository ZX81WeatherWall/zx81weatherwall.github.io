...
(function () {
  ...
  async function refresh() {
    ...
    try {
      ...
      lastFloodMarks = floodMarks;   // stashed so the WEATHER loop frames carry the same flood icons
  ...
  function tapeJob(t, f, wx, o) {
    ...
    return {
      ...
      precipByte: L.precipToByte(wx.precipMm),
      cat: o.cat != null ? (o.cat & 0xff) : L.weatherCodeToCat(wx.weatherCode || 0),
      seaState,
      ...
      for (let f = 0; f < availW; f++) {
        ...
        for (let t = 0; t < lastData.length; t++) {
          ...
          frameField[t] = !wx ? wx : Object.assign({}, wx, {
            ...
            pressureHpa: histAt(wx.pressHist, f, wx.pressureHpa),
          });
        }
        // SAME front-page preparation as the still (owner 2026-09-25: "wx changes during its
        // turn" — the loop frames skipped it, so the slot opened on the clean chart and then
        // jumped to grey land fill + a big icon on every tile). Per frame, from that hour's
        // readings: isobars -> curated pictograms / H-L / temp markers, then the same flood
        // icons + fire zones the still carries (fires/floods are current lists — incidents
        // move over days; same convention as the FIRE loop's dedicated feeds).
        if (GW.computeIsobars) GW.computeIsobars(frameField, N);
        if (GW.computeWeatherFrontPage) GW.computeWeatherFrontPage(frameField, [], [], null, landMasks);
        if (GW.applyEmergencies) GW.applyEmergencies(frameField, lastFloodMarks, N);
        if (GW.applyFireZones) GW.applyFireZones(frameField, lastFireZones, N, landMasks);
        ...
        for (let t = 0; t < N * N; t++) {
          ...
          const cyc = hd ? headCycByte(hd)
            ...
            : 0;
          // Mirror the still's WEATHER job: tier-NONE tiles seat their pictogram via glyphPos,
          // pictograms/markers come from the front-page pass (phen [] = blank tile, NOT null,
          // which would stamp the legacy big glyph everywhere), iso = temp-marker/flame bits,
          // land drawn as white paper + black coast (outlineMask), category from glyphCat.
          const tier = L.cycTierOf(cyc);
          const cycOut = (tier === L.CYC.NONE && fw.glyphPos)
            ? L.cycloneToByte(L.CYC.NONE, fw.glyphPos | 0, 0, 0, 0) : cyc;
          jobs.push(tapeJob(t, f, fw, { cyc: cycOut,
            iso: (fw.tempMarker != null ? 0x40 : 0) | (fw.fireMark ? 0x20 : 0), periph: 0,
            phen: (tier < L.CYC.CYCLONE && Array.isArray(fw.phen)) ? fw.phen : null,
            cat: fw.glyphCat != null ? fw.glyphCat : null,
            outlineMask: outlineMasks[t],
            page: L.PAGE.WEATHER,
